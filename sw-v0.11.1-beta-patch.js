...
const STATIC_CACHE = 'inflation-insights-static-v0.11.1-beta-patch-1';
const CACHE_PREFIX = 'inflation-insights-';
const RELEASE_URL = './v0.11.1-Beta-Patch.html';
const APP_SHELL = [
  './',
  './index.html',
  RELEASE_URL,
  './manifest-v0.11.1-beta-patch.webmanifest',
...
async function navigationResponse(request){
  try{
    ...
    if(response.ok){
      ...
      if(url.origin === self.location.origin && /(?:index\.html|v0\.11\.1-Beta-Patch\.html)$/.test(url.pathname)){
        const cache=await caches.open(STATIC_CACHE);
...
self.addEventListener('message',function(event){
  ...
  if(event.data && event.data.type === 'GET_VERSION' && event.source){
    event.source.postMessage({type:'VERSION',version:'v0.11.1-Beta-Patch',cache:STATIC_CACHE});
  }
});
