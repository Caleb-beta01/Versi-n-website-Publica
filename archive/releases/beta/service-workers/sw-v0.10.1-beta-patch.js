...
const STATIC_CACHE = 'inflation-insights-static-v0.10.1-beta-patch-1';
const CACHE_PREFIX = 'inflation-insights-';
const RELEASE_URL = './v0.10.1-Beta-Patch.html';
const APP_SHELL = [
  './',
  './index.html',
  RELEASE_URL,
  './manifest-v0.10.1-beta-patch.webmanifest',
...
self.addEventListener('activate',function(event){
  event.waitUntil((async function(){
    ...
    await Promise.all(names.filter(function(name){ return name.startsWith(CACHE_PREFIX) && name !== STATIC_CACHE; }).map(function(name){ return caches.delete(name); }));
...
async function cachedRelease(){
  const cache=await caches.open(STATIC_CACHE);
  return (await cache.match(RELEASE_URL,{ignoreSearch:true})) || (await cache.match('./index.html',{ignoreSearch:true}));
}
...
async function navigationResponse(request){
  try{
    ...
    if(response.ok){
      ...
      if(url.origin === self.location.origin && /(?:index\.html|v0\.10\.1-Beta-Patch\.html)$/.test(url.pathname)){
        const cache=await caches.open(STATIC_CACHE);
