...
const STATIC_CACHE = 'inflation-insights-static-v0.9.0-beta-1';
const CACHE_PREFIX = 'inflation-insights-';
const RELEASE_URL = './v0.9.0-Beta.html';
const APP_SHELL = [
  './',
  './index.html',
  RELEASE_URL,
  './manifest-v0.9.0-beta.webmanifest',
...
async function navigationResponse(request){
  try{
    ...
    if(response.ok){
      ...
      if(url.origin === self.location.origin && /(?:index\.html|v0\.9\.0-Beta\.html)$/.test(url.pathname)){
        const cache=await caches.open(STATIC_CACHE);
